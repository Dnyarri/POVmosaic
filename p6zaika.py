...
resultfile.writelines(['/*\n',
                       ...
                       'Version: 3.7\n',
                       'Description: Mosaic picture consisting from solid triangle prisms, C6 symmetry (honey comb) packing whose height corresponds to source image brightness\n',
                       'Author: Automatically generated by p6zaika program, based on AmphiSoft POV Sphere Mosaic plug-in, see POVRay Mosaic project at\n',
                       'https://github.com/Dnyarri/POVmosaic\n',
                       'https://gitflic.ru/project/dnyarri/povmosaic\n',
...
resultfile.writelines(['\n// Object thingie\n',
                       ...
                       '#declare thingie = prism {linear_sweep linear_spline 0, 1, 7, <-0.5, 0.5*revsqrtof3>, <0,revsqrtof3>, <0.5, 0.5*revsqrtof3>, <0.5,- 0.5*revsqrtof3>, <0,-revsqrtof3>, <-0.5,- 0.5*revsqrtof3>, <-0.5, 0.5*revsqrtof3> rotate x*90}\n',
                       ...
                       '#declare xyzsize = 1.0;         // x,y,z-Size value for all thingies, Default 1.0\n',
                       '#declare zmultiplier = 1.0;     // z-Size multiplier for all thingies rescaling, Default 1.0\n',
...
# Height of tringie with 2*0.5 sides, that is the distance between two 0.5 internal radius thingies as defined by thingie. sqrt(3) = 1.732... hardcoded to remove math export
thingieheight = 0.5 * 1.7320508075688772935274463415059

translatestring = ' '
oddtranslatestring = ' '  # no offset
# 0.5 x offset for honeycomb packing
...
for y in range(0, Ycount, 1):
    ...
    for x in range(0, X, 1):
        ...
        r = float(src(x, y*thingieheight, 0))/maxcolors
        g = float(src(x, y*thingieheight, 1))/maxcolors
        b = float(src(x, y*thingieheight, 2)) / \
        ...
        # further adding random to z-scaling, keeping 0..1 range
        zsize = zsize - 0.05*yarkost*random()
        ...
        if (a > tobeornottobe):
            ...
            resultfile.write(
                f' scale <xyzsize, xyzsize, zmultiplier*xyzsize*{zsize}>')
            resultfile.write(translatestring)
            resultfile.write(f'translate <{x}, {
                             y*thingieheight}, displace_factor*{zdisplacement}>')
...
resultfile.write(
    'object {thething}     // good place to change whole object properties\n\n')
