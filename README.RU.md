...
- **b4zaikaS** - конвертирует каждый пиксель в 3D-параллелепипед ("кубик"), "кубики" упакованы в плоскую решётку с *квадратной* сеткой (симметрия C<sub>4</sub>, разбиение 4<sub>4</sub>). Высота верхней грани "кубика" над исходной плоскостью (размер по вертикали) зависит от яркости исходного изображения, что добавляет объём (наподобие очень грубой карты высот).
...
- **p6zaikaR** -конвертирует каждый пиксель в шестигранную 3D-призму ("карандаш"), "карандаши" упакованы в плоскую решётку с *шестиугольной* сеткой (симметрия C<sub>6</sub>, "**пчелиные соты**", разбиение 6<sub>3</sub>), высота "карандаша" в "сотах" зависит от яркости исходного изображения, что добавляет объём (наподобие очень грубой карты высот из упакованных карандашей).
