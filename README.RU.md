...
- **b4zaikaS** - конвертирует каждый пиксель в 3D-параллелепипед ("кубик"), "кубики" упакованы в плоскую решётку с *квадратной* сеткой (симметрия C<sub>4</sub>). Высота верхней грани "кубика" над исходной плоскостью (размер по вертикали) зависит от яркости исходного изображения, что добавляет объём (наподобие очень грубой карты высот).
